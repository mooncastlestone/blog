...
description: As a developer, deploying an Express server to the cloud might seem challenging, but with AWS EC2, it’s quite straightforward. In this guide, I’ll show you how to deploy your Express server on an EC2 instance.
category: Infra
...
As a developer, deploying an Express server to the cloud might seem challenging, but with AWS EC2, it’s quite straightforward.
...
4. <strong>Step 2: Connect to Instance and Set Up an Elastic Ip</strong>

   a. Connecting to Your EC2 Instance

   b. Assigning An Elastic IP
...
✅ <strong>Why is choosing a region important ?</strong>
...
✅ <strong>How to choose a region ?</strong>
...
In this guide, I will use <strong><u>Ubuntu Server 24.04 LTS</u> (Free tier eligible)</strong>
...
In this guide, I will use <strong><u>t2.micro</u> (Free tier eligible)</strong>
...
✅ <strong>Steps to create and use a key pair</strong>
...
1. Choose <strong><u>RSA</u></strong> or <strong><u>ED25519</u></strong> (RSA is commonly used and widely supported).
2. Select the private key file format

   a. <strong><u>.pem</u></strong> for macOS/Linux

   b. <strong><u>.ppk</u></strong> for Windows (used with PuTTY)

3. <strong>Click "Create key pair"</strong> and save this file in a secure location.

<br />

In this guide, I will check <strong>RSA</strong> and <strong>.pem</strong>
...
5. <strong>Review and Create</strong>
   - Check your Security Group settings.
   - Click <strong>"Create"</strong> to finish setting up your Security Group.
...
## Step 2: Connect to Instance and Setting Up an Elastic IP

<br />
### 1️⃣ Connecting to your EC2 Instance
...
3. <strong>Choose "EC2 Instance Connect"</strong>
   - In the <strong>"Connect to instance"</strong> tab, select "EC2 Instance Connect"
...
### 2️⃣ Set Up an Elastic IP
Now that your are connected to your instance, it is important to set up an <strong>Elastic IP.</strong>

<br />

The reason to set up an Elastic IP is that <strong>the IP address of an EC2 instance changes whenever the instance restarted.</strong> This happens because IPv4 addresses are limited, and AWS reassigns these addresses to manage them efficiently.
...
1. <strong>Allocate an Elastic IP address</strong>
    ...
    ![thumbnail](https://res.cloudinary.com/dr4ka7tze/image/upload/v1720926439/%EB%B8%94%EB%A1%9C%EA%B7%B8/AWS_EC2:A_Step_By_Step_Guide/16_gbyz1y.png)
...
2. <strong>Associate the Elastic IP Address With Your Instance</strong>
    ...
    - Click "Associate" to bind the Elastic IP address to your EC2 instance.
...
🚨 <strong>Important</strong>: Remember that AWS charges for Elastic IP addresses that are allocated but not associated with a running instance. Make sure to release the Elastic IP if you no longer need it to avoid unnecessary costs.

<br />

By following these steps, you can successfully launch your EC2 instance, connect to it, and set up an Elastic IP for stable and persistent access.
...
3. <strong>Verity the installation</strong>
